...
Since we are using a **Monorepo**, a change in one tool shouldn't necessarily trigger a release for another unless they share code. However, for simplicity in the MVP phase (Day 5), we will rebuild all artifacts on every push to `main`.
